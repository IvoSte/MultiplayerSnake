...
class Env_agent(Agent):
    def __init__(self, x_pos, y_pos, size, color):
        ...
        self.agent_effect_step_size = config["COSMETIC"]["AGENT_EFFECT_STEP_SIZE"]
    ...
    def update_big_wave(self):
        ...
        self.color = fade_colors(
            self.top_color,
            self.base_color,
            255 / self.agent_effect_step_size,
            (255 - self.intensity) / self.agent_effect_step_size,
        )
    ...
    def decrease_intensity(self, decrease=None):
        if decrease is None:
            decrease = self.agent_effect_step_size
